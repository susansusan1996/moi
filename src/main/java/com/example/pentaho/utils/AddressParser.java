...
public class AddressParser {
    ...
    public AddressParser(IbdTbAddrDataNewRepository ibdTbAddrDataNewRepository) {
        this.ibdTbAddrDataNewRepository = ibdTbAddrDataNewRepository;
    }

    public Address parseAddress(String origninalAddress) {
        ...
        List<String> areaList = ibdTbAddrDataNewRepository.queryAllArea(); //大概會有一百多萬筆，不知道會不會掛掉
        String dynamicAreaPart = String.join("|", areaList);
        ...
        String numFlr2 = "(?<numFlr2>[之][0-9A-Z\\uFF10-\\uFF19一二三四五六七八九十]|[0-9A-Z\\uFF10-\\uFF19一二三四五六七八九十][號樓之區棟]|[0-9A-Z\\uFF10-\\uFF19一二三四五六七八九十](?!室))?";
        String numFlr3 = "(?<numFlr3>[之][0-9A-Z\\uFF10-\\uFF19一二三四五六七八九十]|[0-9A-Z\\uFF10-\\uFF19一二三四五六七八九十][號樓之區棟]|[0-9A-Z\\uFF10-\\uFF19一二三四五六七八九十](?!室))?";
        String numFlr4 = "(?<numFlr4>[之][0-9A-Z\\uFF10-\\uFF19一二三四五六七八九十]|[0-9A-Z\\uFF10-\\uFF19一二三四五六七八九十][號樓之區棟]|[0-9A-Z\\uFF10-\\uFF19一二三四五六七八九十](?!室))?";
        String numFlr5 = "(?<numFlr5>之[0-9])?";
        String room = "(?<room>[0-9A-Z\\uFF10-\\uFF19一二三四五六七八九十]室)?";
        String addrRemains = "(?<addrRemains>.+)?";
        String pattern = "(?<zipcode>(^\\d{5}|^\\d{3})?)(?<county>[臺台][北中南東]([縣市])?" + dynamicCountyPart + ")?" + town + village + neighbor + road + area + lane + alley + subAlley  + numFlr1 + numFlr2 + numFlr3 + numFlr4 + numFlr5  + room + addrRemains;
